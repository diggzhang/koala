...
LOGFILE=/tmp/localPullRestoreKoala.log
...
ssh -t -t master@bd.yangcong345.com '/bin/bash /Backup/koalaDailyEventsBackup/remoteDump.sh' && \
...
rm $TODAYYEAR$TODAYMONTH$TODAYDAY.7z
scp master@bd.yangcong345.com:/Backup/koalaDailyEventsBackup/$TODAYYEAR$TODAYMONTH$TODAYDAY.7z ./ >> $LOGFILE && \
echo "5. end to scp today 7z file" >> $LOGFILE && \

echo "6. start to unarchive 7z file" >> $LOGFILE && \
7za x $TODAYYEAR$TODAYMONTH$TODAYDAY.7z >> $LOGFILE && \
...
mongorestore --db dailyEvents --collection events ./koala/"db"$TODAYYEAR$TODAYMONTH$TODAYDAY.bson >> $LOGFILE && \
mongo dailyEvents --eval "db.events.find({}).sort({'_id':-1}).limit(1)" >> $LOGFILE
